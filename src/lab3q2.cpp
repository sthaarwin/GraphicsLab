...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(800, 600, "Lab 3 Q2 - 2D Translation", NULL, NULL);
    if (!window) {
    ...
    Mat3 T1 = translation(0.4f, 0.0f);       // Translate right
    Mat3 T2 = translation(0.0f, 0.5f);       // Translate up
    Mat3 T3 = translation(0.5f, -0.4f);      // Translate right-down
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Translated versions
        auto tri1 = applyTransform(triangle, T1);
        drawPolygon(tri1, 1.0f, 0.0f, 0.0f);  // Red

        auto tri2 = applyTransform(triangle, T2);
        drawPolygon(tri2, 0.0f, 1.0f, 0.0f);  // Green

        auto tri3 = applyTransform(triangle, T3);
